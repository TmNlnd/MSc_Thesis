...
def train(data_warehouse, i):
    ...
    my_file = Path(join(Base_dir,
        '2D-models/best-CNN-92-{}-{}-{}-{}-{}.h5'.format(epochs, seq_len, number_filter, drop, i)))
    filepath = join(Base_dir, '2D-models/best-CNN-92-{}-{}-{}-{}-{}.h5'.format(epochs, seq_len, number_filter, drop, i))
    if my_file.is_file():
        print('loading model')
...
def run_cnn(data_warehouse, order_stocks):
    ...
    cnn_results_f1.to_csv(join(Base_dir, '2D-models/new CNN 92 results.csv'), index=False)                                    # Saves the results in a new CSV
...
def run_svm(data_warehouse, order_stocks):
    ...
    svm_results_f1.to_csv(join(Base_dir, '2D-models/new SVM 92 results.csv'), index=False)                                    # Saves the results in a new CSV
    print("F1-results: ")
...
def train(data_warehouse, i):
    ...
    my_file = Path(join(Base_dir,
        '2D-models/best-ANN-92-{}-{}.h5'.format(epochs, i)))
    filepath = join(Base_dir, '2D-models/best-ANN-92-{}-{}.h5'.format(epochs, i))
    if my_file.is_file():
        print('loading model')
...
def run_ann(data_warehouse, order_stocks):
    ...
    ann_results_f1.to_csv(join(Base_dir, '2D-models/new ANN 92 results.csv'), index=False)                                    # Saves the results in a new CSV
